...
在 `cloud` 模式下，管理员后台文件路径浏览的是公网服务器文件系统，不是用户本地电脑。普通用户后台会通过已绑定的本地 Agent 浏览和选择自己账号的可访问路径；手机端文件菜单只显示当前账号保存后的可访问路径。

## 九、安全建议

- 公网服务器只运行 `cloud` 模式，不在公网服务器上启用真实执行 Runtime。
- 本地 Agent 使用普通用户运行，不要用 root。
- `policy.allow_paths` 只配置本机默认可访问项目目录，不要配置 `/`；普通账号保存自己的允许路径，解绑客户端会清空该账号已有路径。
